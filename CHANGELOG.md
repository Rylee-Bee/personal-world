...
- 2026-09-20 — **build-vnext.sh parameterized (pre-existing hygiene red).** tests/test_public_safety.py flagged a hardcoded `/home/rylee/...` default; now `VNEXT_SOURCE` is required and fails closed. Remaining known red: `.fossil/cache.json` (tracked cache with absolute paths — owner decision pending: untrack vs scrub), and rylee_lore's pre-existing deterministic-test reds.
