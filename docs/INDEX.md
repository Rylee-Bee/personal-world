...
| [STATION-MAP.md](STATION-MAP.md) | Canonical map | The Station's logical deck plan: eight decks (Today, Systems, Projects, Journal, News, Interests, Records, Settings), each named for its job, with its resident — or an icon (Settings). |
| [Station design concepts](../design/concepts/README.md) | Concepts, not implementation | The Station's design pages: the index, the eight layout packets, the dialect sheet, and **Today** at product scale. Static pages that reference the canonical art rather than copying it. |
